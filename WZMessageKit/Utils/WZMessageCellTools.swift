...
class WZMessageCellTools {
  ...
  class func calculateSize(with text: String, fontSize: CGFloat, maxWidth: CGFloat) -> CGSize {
    ...
    let textWidthInOneline = (text as NSString).size(attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: fontSize)]).width
    
    let textSize = (text as NSString).boundingRect(with: maxSize, options: .usesLineFragmentOrigin, attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: fontSize)], context: nil).size
  ...
  class func asynCalculateTextSize(with text: String, fontSize:CGFloat, maxWidth: CGFloat, completion: @escaping (_ size: CGSize) -> ()) {
  ...
  fileprivate class func createTextKey(with text: String, fontSize: CGFloat, maxWidth: CGFloat) -> Int {
